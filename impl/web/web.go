...
func statusToLabel(status core.TargetStatus) string {
	switch status {
	case core.TargetStatusOk:
		return "OK"
...
func statusToClass(status core.TargetStatus) string {
	switch status {
	case core.TargetStatusOk:
		return "ok"
	case core.TargetStatusError:
...
func handleGetIndex(instance *core.Instance) func(http.ResponseWriter, *http.Request) {
...
func RunWeb(config config.Web, instance *core.Instance) error {
